...
const deploymentPyServer = new k8s.apps.v1.Deployment(pythonAppName,
    {
        metadata: {
            namespace: namespaceName,
            labels: pyServerLabels,
        },
        spec: {
            replicas: 1,
            selector: { matchLabels: pyServerLabels },
            template: {
                metadata: {
                    labels: pyServerLabels,
                },
...
// Export the Deployment name
export const pythonAppDeploymentName = deploymentPyServer.metadata.apply(m => m.name);

// Create a LoadBalancer Service for the PythonAPP Deployment
const servicePythonApp = new k8s.core.v1.Service(pythonAppName,
    {
        metadata: {
            labels: pyServerLabels,
            namespace: namespaceName,
...
// Export the Service name and public LoadBalancer endpoint
export const pyhtonAppServiceName = servicePythonApp.metadata.apply(m => m.name);
export const pyhtonAppServicePublicIP = servicePythonApp.status.apply(s => s.loadBalancer.ingress[0].ip)